<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-xl font-black text-base-content">Pedidos de Compra</h2>
      ...
      <table class="table erp-table w-full">
        ...
        <tbody>
          ...
          <tr v-for="item in items" :key="item.id">
            ...
            <td class="font-semibold text-base-content">{{ item.supplier?.legal_name ?? item.supplier_id }}</td>
    ...
    <DrawerPanel v-model="drawerOpen" :title="form.id ? 'Editar Pedido' : 'Novo Pedido'">
      <div class="space-y-4">
        <fieldset class="fieldset">
          ...
          <select v-model="form.supplier_id" class="select border border-base-300 w-full">
        ...
        <fieldset class="fieldset">
          ...
          <input v-model="form.order_number" type="text" placeholder="Automático se vazio" class="input border border-base-300 w-full" />
        </fieldset>
        <div class="grid grid-cols-2 gap-3">
          <fieldset class="fieldset">
            <legend class="fieldset-legend">Data Emissão</legend>
            <input v-model="form.issued_at" type="datetime-local" class="input border border-base-300 w-full" />
          </fieldset>
          <fieldset class="fieldset">
            <legend class="fieldset-legend">Entrega Prevista</legend>
            <input v-model="form.expected_delivery_date" type="date" class="input border border-base-300 w-full" />
          </fieldset>
        </div>
        <fieldset class="fieldset">
