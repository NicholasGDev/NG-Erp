<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <button class="btn btn-brand btn-sm rounded-full" @click="openModal()">+ Novo Pedido</button>
    </div>
    ...
    <dialog ref="modalRef" class="modal">
      <div class="modal-box max-w-lg">
        <h3 class="font-black text-lg mb-4">{{ form.id ? 'Editar' : 'Novo' }} Pedido</h3>
        <div class="space-y-3">
...
function openModal(item = null) { form.value = item ? { ...item, data_emissao: item.data_emissao?.slice(0,16) } : { ...emptyForm }; modalRef.value?.showModal(); }
async function save() {
  saving.value = true;
