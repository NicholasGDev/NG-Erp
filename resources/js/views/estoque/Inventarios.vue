<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <button class="btn btn-brand btn-sm rounded-full" @click="openModal()">+ Novo Inventário</button>
    </div>
    ...
    <dialog ref="modalRef" class="modal">
      <div class="modal-box max-w-lg">
        <h3 class="font-black text-lg mb-4">{{ form.id ? 'Editar' : 'Novo' }} Inventário</h3>
        <div class="space-y-3">
          ...
          <fieldset class="fieldset">
            ...
            <textarea v-model="form.observacoes" class="textarea textarea-bordered w-full" rows="2" />
          </fieldset>
        </div>
...
function openModal(item = null) { form.value = item ? { ...item, data_inicio: item.data_inicio?.slice(0,16), data_fim: item.data_fim?.slice(0,16) ?? '' } : { ...emptyForm }; modalRef.value?.showModal(); }
async function save() {
  saving.value = true;
